...
//     Validates data/profiles.json, data/glossary.json and
//     data/concept-cards.json. Exits 1 if anything fails.
//
...
//     Used against the fixtures in data/fixtures/.
//
...
async function validateFile(relativePath, schemaKey) {
  ...
  if (data.length === 0) {
    return { relativePath, ok: false, errors: ["array is empty — at least one entry is expected"] };
  }
...
async function main() {
  ...
  const results = await Promise.all([
    validateFile("data/profiles.json", "profile"),
    validateFile("data/glossary.json", "glossary"),
    validateFile("data/concept-cards.json", "concept-card"),
  ]);
